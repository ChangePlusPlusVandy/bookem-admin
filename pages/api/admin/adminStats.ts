...
async function handleAllStats(req: NextApiRequest, res: NextApiResponse<Stats | Error>) {
  try {
    ...
    res.status(200).json({ userCount: userCount.count, volunteerHours: volunteerHours.totalHours, eventCount: eventCount.count });
  } catch (e) {
